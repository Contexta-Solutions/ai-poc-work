...
// The demo leads with Orthopaedics (that's where the 15 templates are), so both
// pickers default there. Both stay fully selectable.
const DEFAULT_SPECIALTY = 'orthopedics';
const DEFAULT_DOCTOR = 'Dr. Nikhil Verma'; // Orthopedics — Joint Replacement Surgery

// One prescription cell. Reads as plain text until clicked, then becomes an
// input in place -- same feel as editing any other line in the note.
const RxCell = ({ value, editing, onStartEdit, onChange, onDone, placeholder = '—', bold = false }) => {
  if (editing) {
    return (
      <input
        autoFocus
        ...
        className="w-full min-w-[80px] text-[13px] text-slate-700 bg-white border border-teal-400 rounded px-1.5 py-0.5 outline-none focus:ring-2 focus:ring-teal-500/20"
      />
    );
  }
  return (
    <span
      onClick={onStartEdit}
      title="Click to edit"
      className={`block cursor-text rounded px-1.5 py-0.5 -mx-1.5 hover:bg-teal-50/70 hover:ring-1 hover:ring-teal-200 transition-all ${
        value ? (bold ? 'font-medium text-slate-700' : 'text-slate-600') : 'text-slate-300'
      }`}
    >
      {value || placeholder}
    </span>
...
export default function VisitNotesApp() {
  ...
  const [editingRxCell, setEditingRxCell] = useState(null); // { itemIndex, field }
  ...
  // Doctor roster comes from the backend, so Visit Notes and the ChatBot can't
  // drift apart.
  useEffect(() => {
    fetch(`${API_URL}/api/doctors`)
      .then(r => (r.ok ? r.json() : null))
      .then(d => { if (d?.doctors?.length) setDoctors(d.doctors); })
      .catch(() => { /* offline: keep the typed-in default */ });
  }, []);
  ...
  const handlePrint = async (lang) => {
    if (!documentData || printLang) return;
    setShowPrintMenu(false);
    setPrintLang(lang);

    const langLabel = PRINT_LANGUAGES.find(l => l.code === lang)?.label || lang;
    showNotification(
      lang === 'en' ? 'Generating PDF…' : `Translating to ${langLabel} and generating PDF…`,
      'success'
    );
    ...
    try {
      const res = await fetch(`${API_URL}/api/generate-pdf`, {
        ...
        body: JSON.stringify({
          ...
          clinical_data: documentData.document,
        })
      });
      if (!res.ok) {
        let detail = `Failed to generate PDF (${res.status})`;
      ...
      link.href = url;
      link.download = `EMR_${metadata.patient.replace(/\s+/g, '_')}_${metadata.visit_date}_${lang}.pdf`;
      link.click();
  ...
  // Prescription cells are edited in place, one column at a time.
  const handleRxCellChange = (itemIndex, field, value) => {
    setDocumentData(prev => {
      const doc = { ...prev.document };
      doc.prescription = doc.prescription.map((item, i) => {
        if (i !== itemIndex) return item;
        const next = { ...item, [field]: value };
        const parts = [next.drug, next.dosage, next.frequency, next.duration, next.instructions].filter(Boolean);
        return { ...next, rendered_line: parts.join(' | ') };
      });
      return { ...prev, document: doc };
    });
  };

  // Wipes the dictation AND the note it produced, so you're back to a blank
  // slate rather than staring at a document whose dictation is gone.
  const handleClearAll = () => {
    setVoiceNote('');
    setDocumentData(null);
    setEditingItem(null);
    setEditingRxCell(null);
    setAddingToSection(null);
    setAddItemText('');
    setShowPrintMenu(false);
    showNotification('Dictation and document cleared', 'success');
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-teal-50/30 flex flex-col">
      ...
      {/* ─── TOP NAVBAR ─── */}
      <nav className="sticky top-0 z-50 backdrop-blur-xl bg-white/80 border-b border-slate-200/60 px-3 sm:px-6 py-3">
        <div className="flex items-center justify-between gap-2 max-w-[1800px] mx-auto">
          {/* The wordmark alone is ~150px wide, which is most of a phone's
              navbar. Below md, keep only the Home button. */}
          <div className="flex items-center gap-3 min-w-0">
            <a href="/" className="px-3 py-2 bg-slate-100 text-slate-600 rounded-xl text-sm font-semibold hover:bg-slate-200 transition-all flex items-center gap-1.5 no-underline flex-shrink-0">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
              <span className="hidden sm:inline">Home</span>
            </a>
            <ContextaLogo className="h-[22px] w-auto hidden md:block" />
            <div className="h-5 w-px bg-slate-200 mx-1 hidden md:block"></div>
            <span className="text-xs font-semibold text-teal-700 bg-teal-50 px-2.5 py-1 rounded-full tracking-wide uppercase hidden lg:inline">
              EMR Demo
            </span>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2.5 flex-shrink-0">
            ...
            <div className="relative">
              <button
                ...
                disabled={!documentData || !!printLang}
              ...
              >
                ...
                <span className="hidden sm:inline">{printLang ? 'Generating…' : 'Print PDF'}</span>
                {!printLang && (
                  <svg className="w-3.5 h-3.5 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7" /></svg>
                )}
              </button>

              {showPrintMenu && documentData && !printLang && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => setShowPrintMenu(false)} />
                  <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-xl shadow-xl shadow-slate-300/30 overflow-hidden z-50">
                    <p className="px-4 pt-3 pb-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400">Print language</p>
                    {PRINT_LANGUAGES.map(l => (
                      <button
                        key={l.code}
                        onClick={() => handlePrint(l.code)}
                        className="w-full text-left px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-teal-50 hover:text-teal-700 transition-colors"
                      >
                        {l.label}
                      </button>
                    ))}
                    <p className="px-4 py-2 text-[10px] text-slate-400 border-t border-slate-100 leading-snug">
                      Drug names stay in English.
                    </p>
                  </div>
                </>
              )}
      ...
      {/* ─── MAIN CONTENT ─── */}
      {/* Phone/tablet: dictation on top, generated note below it (col-reverse),
          matching the actual workflow. Desktop: note left, dictation right. */}
      <main className="flex-1 p-3 sm:p-4 lg:p-6 flex flex-col-reverse lg:flex-row gap-4 lg:gap-5 max-w-[1800px] mx-auto w-full min-w-0">
        ...
        {showTemplates ? (
          ...
          {/* Metadata fields — these go on the printed note's header band. */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-6">
            <div className="group">
              <label className="text-[11px] font-semibold text-slate-400 block mb-1.5 uppercase tracking-wider group-hover:text-teal-600 transition-colors">Patient</label>
              <input
                id="input-patient"
                type="text"
                placeholder="Patient name"
                className="w-full border border-slate-200 p-3 rounded-xl text-sm bg-white outline-none text-slate-700 shadow-sm focus:border-teal-400 focus:ring-2 focus:ring-teal-500/10 transition-all"
            ...
            <div className="group">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full border border-slate-200 p-3 rounded-xl text-sm bg-white outline-none text-slate-700 shadow-sm focus:border-teal-400 focus:ring-2 focus:ring-teal-500/10 transition-all appearance-none cursor-pointer pr-10"
                >
                  {doctors.length === 0 && <option value={metadata.doctor}>{metadata.doctor}</option>}
                  {/* Orthopaedics first — it's the default and where the templates are —
                      but every doctor stays selectable. */}
                  {[...doctors]
                    .sort((a, b) =>
                      (a.specialty === 'Orthopedics' ? 0 : 1) - (b.specialty === 'Orthopedics' ? 0 : 1)
                    )
                    .map(d => (
                                  ...
                                  const cell = (field, opts = {}) => (
                                    <td className="px-3 py-2 align-top">
                                      <RxCell
                                        value={item[field]}
                                        bold={opts.bold}
                                        editing={editingRxCell?.itemIndex === idx && editingRxCell?.field === field}
                                        onStartEdit={() => setEditingRxCell({ itemIndex: idx, field })}
                                        onChange={val => handleRxCellChange(idx, field, val)}
                                        onDone={() => setEditingRxCell(null)}
                                      />
                                    </td>
                                  );
                                  return (
                                    <tr key={idx} className="border-t border-slate-100 hover:bg-slate-50/40 transition-colors">
                                      {cell('drug', { bold: true })}
                                      {cell('composition')}
                                      {/* A template dose with a "___" blank keeps its inline slot field;
                                          everything else is a plain editable cell. */}
                                      <td className="px-3 py-2 align-top">
                                        {item.type === 'rx_slot' ? (
                                          <SlotLine
                                          ...
                                          <RxCell
                                            value={item.dosage}
                                            editing={editingRxCell?.itemIndex === idx && editingRxCell?.field === 'dosage'}
                                            onStartEdit={() => setEditingRxCell({ itemIndex: idx, field: 'dosage' })}
                                            onChange={val => handleRxCellChange(idx, 'dosage', val)}
                                            onDone={() => setEditingRxCell(null)}
                                          />
                                        )}
                                      </td>
        ...
        <div className="w-full min-w-0 lg:w-1/3 bg-white/90 backdrop-blur-sm p-4 sm:p-6 md:p-8 shadow-lg shadow-slate-200/40 border border-white/80 rounded-2xl min-h-[340px] sm:min-h-[400px] lg:h-[calc(100vh-7.5rem)] flex flex-col relative overflow-hidden">
          ...
          <textarea
            ...
            className="w-full flex-grow border border-slate-200 p-4 rounded-xl bg-white/60 backdrop-blur-sm resize-none text-[16px] sm:text-[14px] mb-5 focus:ring-2 focus:ring-teal-500/15 focus:border-teal-400 outline-none transition-all leading-relaxed shadow-inner relative z-10 text-slate-700 placeholder:text-slate-300"
            value={voiceNote}
          ...
          <div className="space-y-4 relative z-10">
            ...
            <div className="flex justify-center items-center gap-4 relative">
              ...
              {/* Clear Button — clears the dictation and the generated document */}
              <button
                id="btn-clear-dictation"
                onClick={handleClearAll}
                title="Clear dictation and document"
                disabled={(!voiceNote.trim() && !documentData) || isListening}
